...
// the pen rather than flickering on at random: the capital G of "Giacomo" drawn
// top to bottom, then "iacomo" left to right, a tiny wait, the capital G of
// "Guidotto" top to bottom, then "uidotto" left to right, and finally the closing
// flourish slowing to a stop. Each `d` is a centerline trace whose subpaths are
// ordered and oriented along that stroke's direction, so its single normalized
// stroke-dashoffset reveal walks the stroke the way it was written. Re-tracing the
// source only ever changes these `d`s (and the matching viewBox).
const STROKES = [
  {
    // Capital G of "Giacomo", top to bottom.
      ...
      (entries, obs) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
            ...
            // stroke runs on its own delay/duration/easing, so the pause and the
            // final slow-down live entirely in the STROKES data.
